      ...
      <div className="space-y-8 animate-pulse p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="h-40 rounded-[32px] bg-slate-50 border border-slate-100"
            />
          ))}
        </div>
      </div>
    ...
    <div className="min-h-screen pb-20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        {statCards.map((stat) => (
          <div
            key={stat.label}
            className="bg-white border border-slate-100 rounded-[32px] p-8 hover:shadow-xl hover:shadow-blue-900/5 transition-all group"
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="text-[10px] font-black text-slate-300 uppercase tracking-widest group-hover:text-blue-400 transition-colors">
                Real-time
              </div>
            </div>
          ...
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 rounded-[40px] p-10 relative overflow-hidden group h-full flex flex-col justify-between shadow-sm">
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm font-medium leading-relaxed mb-10">
                Your current application success rate is{" "}
                <span className="text-blue-600 font-bold">
                  {stats?.successRate}% higher
                </span>{" "}
                than the average user in your niche.
              </p>
            </div>
