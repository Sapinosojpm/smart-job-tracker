...
export default function Sidebar({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 z-50 flex h-screen w-64 flex-col overflow-hidden border-r border-slate-200 bg-white shadow-[4px_0_24px_-12px_rgba(15,23,42,0.12)] print:hidden transition-transform duration-300 ease-in-out md:translate-x-0 ${
      ...
      <div className="mt-auto border-t border-slate-100 bg-slate-50/90 px-4 py-4">
        ...
        <div className="flex items-center justify-between mt-3 px-1">
          <p className="text-[10px] text-slate-400 font-medium">v1.2.0-BETA</p>
          <button
            onClick={async () => {
              const { createClient } = await import('@/utils/supabase/client');
              const supabase = createClient();
