    ...
    <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 animate-in fade-in duration-300">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-[40px] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.15)] flex flex-col animate-in zoom-in-95 duration-500 overflow-hidden">
        ...
        <div className="p-8 pb-4 text-center border-b border-slate-50 relative">
          <button onClick={onClose} className="absolute right-6 top-6 p-2 rounded-full hover:bg-slate-100 text-slate-400 transition-colors">
            <X size={20} />
          </button>
          ...
          <h2 className="text-2xl font-black text-slate-900 tracking-tight mb-2">Share your story</h2>
          <p className="text-slate-500 text-[13px] font-medium leading-relaxed">
            How has Smart Job Tracker changed your search?
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-5">
          ...
          >
            {isSubmitting ? <Loader2 className="animate-spin" size={20} /> : <><Sparkles size={18} /> Post Story</>}
          </button>
        </form>

        <div className="p-5 bg-slate-50 text-center">
           <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Publicly visible on landing page</p>
        </div>
      </div>
    </div>
  );
