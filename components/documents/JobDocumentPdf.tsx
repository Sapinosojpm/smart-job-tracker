...
function ResumePdf({ resumeData }: { resumeData: ResumeData }) {
  const skillEntries = Object.entries(resumeData.skills).filter(([, value]) => value.trim());
      ...
      <View style={styles.section}>
        ...
        <View style={styles.entryList}>
          {resumeData.experience.map((exp, index) => (
            <View key={`${exp.title}-${index}`} style={styles.entryBlock} wrap={false}>
              ...
              <View style={styles.bulletList}>
                {exp.bullets.filter((bullet) => bullet.trim()).map((bullet, bulletIndex) => (
                  <View key={`${exp.title}-bullet-${bulletIndex}`} style={styles.bulletRow}>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.entryList}>
          {resumeData.projects.map((project, index) => (
            <View key={`${project.name}-${index}`} style={styles.entryBlock} wrap={false}>
              ...
              <View style={styles.bulletList}>
                {project.bullets.filter((bullet) => bullet.trim()).map((bullet, bulletIndex) => (
                  <View key={`${project.name}-bullet-${bulletIndex}`} style={styles.bulletRow}>
                    <Text style={styles.bulletMark}>•</Text>
                    <Text style={styles.bulletText}>{bullet}</Text>
                  </View>
                ))}
...
export function JobDocumentPdf({
  ...
  return (
    <Document author={resumeData.fullName} creator="Smart Job Tracker" title={title}>
