...
export default function ApplyModal({ job, onClose, onSuccess }: ApplyModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err.message || 'Failed to save application details');
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-8 text-white relative">
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              <h2 className="text-2xl font-bold tracking-tight">Record Application</h2>
              <p className="text-blue-100 text-sm font-medium mt-1">
                {job.title} @ <span className="text-white font-bold">{formData.companyName}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 max-h-[70vh] overflow-y-auto bg-slate-50/30">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              ...
              <div className={`p-5 rounded-2xl border transition-all duration-300 ${formData.skillsMet ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-200'}`}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <CheckCircle2 size={20} className={formData.skillsMet ? 'text-emerald-600' : 'text-slate-400'} />
                    <span className={`text-sm font-bold ${formData.skillsMet ? 'text-emerald-700' : 'text-slate-600'}`}>
                      Requirements Met?
                    </span>
                  </div>
          ...
          <div className="mt-8 space-y-2">
            ...
            <textarea 
              placeholder="Write down your thoughts about the company or interview notes..."
              rows={3}
